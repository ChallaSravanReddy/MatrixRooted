  ...
  return (
    <div className="flex h-screen bg-[#0d0d0d] text-[#ffffff] overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="h-16 border-b border-[#00ffcc]/10 bg-[#141414] flex items-center gap-4 px-6 shrink-0 shadow-none">
          <button 
          ...
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} transition={{ type: "spring", stiffness: 400, damping: 25 }}>
            <Button variant="outline" size="icon" onClick={() => router.push('/dashboard')} className="rounded-[8px] h-8 w-8 border-[#00ffcc]/20 shadow-none">
              <ArrowLeft size={16} className="text-[#00ffcc]" />
            </Button>
          </motion.div>
          <h2 className="text-xs font-bold text-[#ffffff]">My Enrolled Course Classes</h2>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-[24px] md:p-[48px] pb-20 max-w-7xl mx-auto w-full">
          ...
          >
            {enrollments.length > 0 ? (
              enrollments.map((enroll) => {
                ...
                return (
                  <motion.div 
                    ...
                    key={enroll.id} 
                    className="flex flex-col bg-[#141414] border border-[#00ffcc]/20 rounded-[12px] p-[20px] hover:border-[#00ffcc]/40 transition-colors shadow-none group"
                  >
                    <div className="flex items-center justify-between border-b border-[#00ffcc]/10 pb-[12px] mb-[12px]">
                       <div className="w-8 h-8 rounded-[8px] bg-[#00ffcc]/5 border border-[#00ffcc]/10 flex items-center justify-center text-[#00ffcc]">
                          <BookOpen size={14} />
                       </div>
                       {enroll.certification_status === 'approved' ? (
                         ...
                         <div className="flex items-center gap-1 text-[9px] font-bold text-[#00ffcc] uppercase tracking-wider bg-[#00ffcc]/5 px-2 py-0.5 rounded-[4px] border border-[#00ffcc]/10">
                    ...
                    <p className="text-xs text-[#ffffff]/70 line-clamp-2 mb-[16px] leading-[1.6] font-medium">
                      {enroll.courses?.description}
                    </p>

                    <div className="space-y-[16px] mt-auto">
                      <div className="space-y-1.5 pt-2 border-t border-[#00ffcc]/5">
                      ...
                      <div className="pt-[12px] border-t border-[#00ffcc]/10 space-y-[8px]">
                        <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} transition={{ type: "spring", stiffness: 400, damping: 25 }}>
                          <Button className="w-full rounded-[8px] h-9 font-bold bg-[#00ffcc] text-[#ffffff] hover:bg-[#00e6b8] shadow-none text-xs" onClick={() => router.push(`/dashboard/courses/${enroll.course_id}`)}>
                            Resume Lessons
                          </Button>
                        </motion.div>
              ...
              <motion.div variants={cardVariants} className="col-span-full py-[64px] text-center bg-[#141414] border border-[#00ffcc]/10 rounded-[12px] space-y-[16px]">
                 <div className="w-12 h-12 bg-[#00ffcc]/5 border border-[#00ffcc]/10 rounded-[12px] flex items-center justify-center mx-auto text-[#00ffcc]">
                    <BookOpen size={20} />
                 </div>
                 <div className="space-y-1">
                    <h3 className="text-base font-bold text-[#ffffff]">No Subscriptions Active Yet</h3>
                    <p className="text-xs text-[#ffffff]/70 max-w-sm mx-auto font-medium">Browse available institutional courses to subscribe and kickstart your career learning track.</p>
                 </div>
                 <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} transition={{ type: "spring", stiffness: 400, damping: 25 }} className="pt-2">
                   <Button className="rounded-[8px] bg-[#00ffcc] text-[#ffffff] hover:bg-[#00e6b8] shadow-none h-10 px-6 text-xs font-bold" onClick={() => router.push('/dashboard')}>
